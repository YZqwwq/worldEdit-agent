...
- 短期记忆：最近 8 条 user/AI 消息（最多 4 个完整轮次）。
...
| P1-6 | P1     | Scene 低置信度时直接屏蔽历史世界焦点                                           | 已实施，待对话验收           |

“部分改善”表示当前改造已经提供了基础观测能力，但问题本身尚未闭环，不应视为完成。

## 2026-08-04 当前推进计划

本轮不扩展 Context Planner，只处理已经确认会造成信息重复或连续性丢失的问题：

1. 消除工具结果重复注入。
   - 完整结果只由 `tool-evidence` 提供。
   - `turn-progress` 只保留工具名、成败状态和简短进度，不再复制结果正文。
2. 完成 P0-4 Recall 修正与测试。
   - 按当前短期 Context 已覆盖的具体消息排除原始对话，不再固定排除数据库最近两个 Turn。
   - 将“刚才那个”“之前说的”等自然承接指代与主题 token 判定分开。
   - 补充可重复执行的自动化测试。
3. 扩大正常对话连续性。
   - 默认保留最近 4 个完整轮次（最多 8 条 user/AI 消息）。
   - Instant Perception 使用同一范围。
   - 先通过长对话验收，不引入动态窗口 Planner。
4. 修正 Scene 低置信度时的历史焦点丢失。
   - 不注入低置信度 Scene 结论，也不强行运行新的 WorldFocus。
   - 将上一历史焦点保留为“可能仍相关、本轮未确认”的背景。
   - 只在明确 `scene_shift` 时退出历史焦点。

实施状态：上述四项已完成代码调整；Recall 策略测试、Node 类型检查和 Web 类型检查已通过。超大工具结果的分页协议和 Recall 真实数据库集成验收作为后续验收边界，不阻塞本轮收口。

已确认但不在本轮实施的后续方向：

- 人格可以影响 Agent 是否主动询问和如何表达谨慎，但不决定工具能否执行；强制确认将来放入具体工具内部。
- 已完成生成资源解耦：`maxTokens` 由 Runtime/模型配置提供，人格不再控制 `maxTokens` 和 `topP`，仅在 Runtime 基础温度上提供 `[-0.2, 0.2]` 的有限 `temperature` 偏移。

## 已实施：AI 语义边界与窗口上限共同决定阶段归档

### 问题

当前如果只依赖固定消息数量或归档频率，Stage 容易在话题中间被机械切断；如果让 AI 在每轮自由触发总结归档，又容易频繁调用总结模型，形成大量过碎 Stage。

阶段归档的定位是保存对话连续性和话题分段，不等同于长期重要事件记忆。

### 决策

采用“AI 判断语义边界、Runtime 控制提交、窗口上限强制兜底”的轻量双触发模式。

AI 只表达对话是否形成自然阶段边界，不直接执行归档：

```ts
type ArchiveIntent = { type: 'continue' } | { type: 'episode_boundary'; reason: string }
```

Runtime 只在 archiveBuffer 已有可用片段时请求 ArchiveIntent，再结合边界消息序号和窗口保护上限决定是否生成 Stage。

### 目标行为

1. 待归档消息累计为可用片段后，AI 判断话题、倾诉、关系互动或工作阶段是否已形成自然边界。
2. 语义边界存在时，Runtime 只将边界之前的完整前缀形成 Stage，不等待固定硬上限。
3. AI 一直没有标记边界时，archiveBuffer 达到保护上限后，Runtime 强制归档最早的可用片段，避免无限堆积。
4. AI 频繁标记边界时，未提交的边界进行合并，不为每个信号立即生成独立 Stage。
5. 归档完成后，Agent 再依据人格和行为偏好判断其中是否存在值得形成长期重要事件记忆的内容。

### 职责边界

| 参与方     | 职责                                                         |
| ---------- | ------------------------------------------------------------ |
| AI         | 感知对话是否形成自然阶段边界，并给出简短原因                 |
| Runtime    | 记录、合并边界信号，结合窗口状态决定归档时机，并保证上限兜底 |
| Stage 归档 | 对已闭合的对话片段进行总结，供后续 Recall 使用               |
| 长期记忆   | 独立判断该 Stage 中哪些共同经历值得长期保留                  |

### 最小实施边界

- 复用现有 message sequence、archiveBuffer 和 Stage 生成链路，不新增 Context Planner。
- 只在消息已离开短期窗口且缓冲达到最小片段时请求一次语义边界判断，不每轮追加持久化边界信号。
- 首版只支持 `continue` 和 `episode_boundary`，不增加优先级、分数、预算或复杂调度。
- 边界判断只能返回 Runtime 给定的候选 message sequence，不允许 AI 绕过 Runtime 直接调用 Stage 总结。
- 阶段归档和长期重要事件写入保持两条独立链路。

### 2026-08-06 实施结果

- `PersonaPolicy` 已移除 `shortTermLimit` 和 `archiveThreshold`，PersonaNode 不再改写记忆窗口或归档阈值。
- Runtime 固定保留最近 8 条 user/AI 消息（最多 4 个完整轮次）。
- 待归档缓冲达到 4 条时，轻量模型只判断是否存在自然阶段边界，不直接写入长期记忆。
- 判断到多个合理边界时选择最靠后的完整 AI 消息边界，将过密信号合并为一个 Stage。
- 没有可用语义边界、边界判断失败或缓冲继续增长时，达到 6 条的 Runtime 硬上限即强制归档。
- Runtime 只归档至选定边界的前缀，边界之后的待归档消息保留给下一 Stage。
- Stage 归档仍是压缩历史，不等于独立长期重要事件；长期事件的人格化选择保持为后续独立链路。
- 新增 `test:memory-archive`，覆盖语义边界最小片段、前缀归档、非法 user 边界和 Runtime 硬上限。
...
| P0-2 | 瞬时场景、情绪和人格调制只看到当前用户消息               | 统一读取最近四个完整轮次，供 Scene、UserMood、WorldFocus、PersonaSignal 和 AI Mood 共享          |
| P0-3 | 消息离开短期窗口后，在形成 Stage 前没有读取出口          | 将 archiveBuffer 暴露为待归档记忆层，并由 recall 默认返回原始消息                                |
| P0-4 | Recall 最近轮排除偏移且自然指代会被 n-gram 误判          | 显式排除短期消息与未完成/非聊天 Turn，并独立识别承接指代；策略测试已通过                         |
| P0-5 | 工具完整结果首次消费前被压缩，且可能与进度信息重复       | 首次通过 ToolMessage 完整交付；执行账本仅保留简短行动摘要，未消费 transcript 不重复注入 evidence |
| P1-1 | 感知系统运行诊断进入主模型 Prompt                        | 运行诊断仅保留在 State 和 Trace，主 Agent 只接收有效感知结果                                     |
| P1-6 | Scene 低置信度时丢失历史世界焦点                         | 低置信度时保留为未确认背景，只在高置信度 scene_shift 时退出                                      |
...
- InstantPerception 开始时统一读取一次短期记忆，取最近 8 条已完成消息，并附加当前用户消息。
...
- 本次覆盖最近四个完整轮次的连续性；archive buffer 的读取空窗由 P0-3 通过按需 recall 单独处理。
...
- 原始对话检索会排除当前 MemorySnapshot.shortTerm 已覆盖的具体消息。
- `queued`/`processing`/`failed`/`reverted` 等未完成 Turn 不再进入原始对话候选；`completed` 和 `interrupted` 可作为历史来源。
- 排除消息使用多重集语义，相同正文只消耗最近的对应项，不会误删所有旧重复。
- 新增自然历史指代判定；“刚才”“刚才那个”“之前说的”“按之前那个”“继续”进入近期承接兜底，而“之前讨论的记忆系统”仍进入主题检索。
- 新增 `npm run test:recall`，覆盖指代判定、明确主题、未完成 Turn 排除和历史重复文本保留。
...
3. 已经进入最近 8 条短期 Context 的消息不会再次作为 raw_message 返回。
...
- 工具执行账本只保留简短行动摘要；正在等待首次消费的 Tool transcript 不会同时重复投影到 `tool-evidence` 或 ephemeral Context。
...
- 低于 0.6、`uncertain` 或 Scene detector 失败时，不运行新的 WorldFocus。
- 存在已解析历史焦点时，继续将其注入 Memory Slot Context，并明确标记为“上一轮可能仍相关、本轮未确认”。
- 只有置信度不低于 0.6 的 `scene_shift` 强制关闭历史焦点注入。
- 低置信度 Scene 结论本身仍不进入主 Prompt，避免把不可靠分类当成事实。
