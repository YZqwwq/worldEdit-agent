...
## 人工工作区与手动版本边界（2026-08-20）

当前决定：保持真实 Git 语义，不因自动保存而自动创建正式版本。

- 编辑器停止输入约 3 秒后保存最新工作区，但只有用户点击“创建版本”才形成正式 Commit。
- 已提交文档可以恢复到最近一次正式版本；尚未提交的中间修改不属于版本历史。
- 新建文档在首次创建版本前属于未提交工作区；如果此时又被删除，版本系统不负责恢复。这与 Git 中未跟踪/未提交文件被删除后的行为一致。
- 不把自动安全检查点伪装成正式版本。以后若真实用户需要防误删或恢复未提交内容，应建立独立的“本地历史/自动恢复”能力，不进入 Git Commit 列表，也不改变手动创建版本的含义。
...
- AI Diff 点击定位失败时应在界面内显示原因。目前历史 Diff 有错误提示，AI Diff 只写控制台，用户会感知为点击无响应。

用户影响：全文重写与局部编辑的修改反馈目前不一致，定位失败也缺少可见解释。维护影响：两类写入仍存在重复且不对称的结果组装逻辑。

P2：暂缓事项

- 不因未提交内容不可恢复而增加自动 Commit。
- 是否建立独立本地历史，等待真实使用反馈后再决定。
- 多处编辑原子聚合、通用 Patch 与 Agent 撤销工具继续按实测需求评估。
