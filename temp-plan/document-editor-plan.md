...
- Agent 能识别当前世界和当前文档，并读取已知文档的完整正文。
...
- 每份文档保留独立内容版本；文档只归属于世界，不与人物、国家、地域等应用实例建立归属关系。
...
- 每次提交指向当时完整的世界文档树，树中记录父子关系、标题、顺序和对应内容版本。
...
- 人工编辑停止输入约 3 秒后只自动保存工作区；创建、更新、移动和删除持续进入同一个待提交工作区，只有用户明确点击“创建版本”才形成正式提交。
- 一次拖拽引起的多个同级排序更新使用同一历史会话，不拆成多个提交。
- 进程重启时先恢复并判定所属 Turn，再分类处理暂存变更；运行中的 Agent ChangeSet 与人工工作区均保持暂存，只有终态 Agent Turn 或用户明确创建版本时才会封口。
...
- 人工工作区在启动时保持未提交状态；Agent ChangeSet 只有在所属 Turn 已完成、已中断、失败或取消后才会形成正式文档提交。
...
验收已覆盖：人工工作区、终态 Agent、运行中 Agent、孤儿 ChangeSet 四类启动恢复，以及显式提交和提交幂等性。
