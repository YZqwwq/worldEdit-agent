<template>
  <div
    class="flex w-full flex-shrink-0 gap-4 rounded-[22px] border border-slate-200 bg-white p-4 shadow-[0_8px_24px_rgba(15,23,42,0.06)]"
  >
    <div class="flex flex-grow flex-col gap-3">
      <div class="flex items-center gap-3">
        <button
          type="button"
          class="flex h-12 w-12 items-center justify-center rounded-xl border border-slate-200 bg-slate-50 text-slate-600 transition-colors hover:bg-slate-100"
          :disabled="isLoading"
          title="上传文件"
        ...
        <div class="relative flex-grow">
          <textarea
            ...
            class="h-14 w-full resize-none rounded-xl border border-slate-200 bg-slate-50 py-3.5 pl-5 pr-4 text-sm text-slate-800 placeholder-slate-400 shadow-inner transition-all focus:border-blue-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20"
            :disabled="isLoading"
            @input="handleInput"
      ...
      <div v-if="uploadedFiles.length" class="flex flex-col gap-2">
        <div
          ...
          :key="file.id"
          class="flex items-start justify-between gap-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-xs text-slate-700"
        >
