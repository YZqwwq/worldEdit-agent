...
import { taskLifecycleNode } from './node/tasklifecyclenode/taskLifecycleNode'
import { taskCoordinationNode } from './node/taskcoordinationnode/taskCoordinationNode'
import { shouldContinue } from './endlogic/shouldContinue'
import { routeAfterTaskCoordination } from './endlogic/routeAfterTaskCoordination'
import { withGraphLog } from '../../log/graphlog'

// 注入状态维持实例
export const agent = new StateGraph(MessagesState)
  .addNode('personaNode', withGraphLog('personaNode', personaNode))
  .addNode('taskLifecycleNode', withGraphLog('taskLifecycleNode', taskLifecycleNode))
  .addNode('taskCoordinationNode', withGraphLog('taskCoordinationNode', taskCoordinationNode))
  ...
  .addEdge('personaNode', 'taskLifecycleNode')
  .addEdge('taskLifecycleNode', 'taskCoordinationNode')
  .addConditionalEdges('taskCoordinationNode', routeAfterTaskCoordination, ['contextNode', END])
